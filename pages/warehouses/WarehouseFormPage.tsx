...
const WarehouseFormPage = () => {
  ...
  const breadcrumbs = [
    ...
    { name: isEditing ? 'Edit Warehouse' : 'New Warehouse', href: isEditing ? `/warehouses/edit/${id}` : '/warehouses/new' },
  ];

  return (
    <div>
      <PageHeader title={isEditing ? 'Edit Warehouse' : 'Create New Warehouse'} breadcrumbs={breadcrumbs} />
      
      <div className="bg-white dark:bg-gray-800 shadow-md rounded-lg p-6">
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label htmlFor="name" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Warehouse Name</label>
